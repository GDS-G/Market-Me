...
**Status: final local release gate and Google-document readback complete; reviewed feature/main Linux CI and public-source publication remain pending.** The latest fully verified public checkpoint remains 1.25 until those external observations complete. Release 1.26 adds a read-only, exact receipt-to-command handoff without changing the 116-migration schema or increasing preparation authority.
...
- The Google development parent and all five child tabs contain the 1.26 design, variables/lifetimes, build/recovery, security and release evidence. Connector readback at revision `ANLCKQm1MNZWjmKnwIUXQ84l2HPKN5k2S8-73oBxfPs8bQkWFwfT-1ENuVY-sYLzgSRxCjObqQ-wHYe-gnwIPFLw-pPFK87YSjv85BoN3Q` verifies each new heading/body-list role, the immediately following 1.25 section and the unchanged 30-tab title/order/parent topology. The skill's advisory file-backed trusted-read bridge rejected the Windows absolute-path convention, so verification used native connector structure/text reads; no rendered-PDF visual-layout claim is made. Feature-branch CI, `main` CI and public-release evidence are intentionally not claimed until observed.

...
## 1.22.0 - Review-first campaign preparation (acceptance in progress)

**Status: local suite, authenticated browser and production build/smoke verified; cloud acceptance pending.** Metadata is synchronized at 1.22.0 on `codex/campaign-starter`. This release creates reviewed planning material, not an executable publishing plan or a complete beginner workflow. No production deployment, provider acceptance or signed distribution is implied.
...
- Cloud verification remains pending. The Google development parent and five children contain the 1.22 architecture, fields/collections, build/recovery rules and compatibility limits, explicitly labeling pending gates. Native readback verifies new headings/lists/inherited typography and preserves every prior development section, all conceptual content and the complete 30-tab hierarchy.

...
## 1.26.0 - Exact approval-to-preparation handoff

**Status: final local release gate, Google-document readback and exact implementation feature CI complete; reviewed documentation feature CI, `main` publication and identical `main` CI remain pending.** The latest fully verified public checkpoint remains 1.25 until those external observations complete. Release 1.26 adds a read-only, exact receipt-to-command handoff without changing the 116-migration schema or increasing preparation authority.

- Adds `SourcePreparationRepository.getSourcePreparationCommandForApproval(workspaceId, contentPackageId, approvalId, actorUserId)`. All four identifiers are canonical UUID inputs. The query joins current workspace membership and matches exact workspace, Content Package and globally unique triggering approval; it never scans the 20-row recent-history list and returns `undefined` for historical approvals that did not enqueue.
- Reuses the minimized `SourcePreparationCommandView`. Approval pages receive only status, binding/package revision, attempt count, optional due time and bounded safe error, optional preparation receipt ID and creation time. Command UUID, exact approval/fingerprint, writer, idempotency key, lease, Campaign ID and snapshots remain server-only.
- Adds a shared status renderer for pending, processing, retryable failure, stopped/dead-letter and completed states. Completed state links only to the immutable preparation receipt; terminal state offers no requeue control. The exact approval receipt supplies a canonical same-page refresh and a Smart Source configuration link.
- Corrects a race-sensitive UI statement: a binding may be enabled or disabled after the review page loads and before approval commits. Only the approval transaction is authoritative, so a warning is shown for either loaded state, stays conditional and directs the operator to the immutable receipt. Absence of a command is final for that receipt rather than an implicit pending state; later configuration never backfills it.
- Adds no table, migration, worker behavior, template semantic, global cache, environment variable, provider call, credential access, approval, finalization, activation, scheduling, publication action or send. Frozen migrations 0113–0116 remain byte-identical; release readiness remains 116 migrations ending at 0116.
- The full local suite passes **2,020 distinct TypeScript tests across 118 files without skips**: web 634/37 files, workflow-worker 68/5, companion protocol 4/1, connectors 166/10, database 958/48, domain 65/3, generation 29/4, ingestion 29/5, media 24/2 and workflows 43/3. Coverage includes pre-binding absence, exact pending/completed lookup, byte-identical reapproval separation, mismatched package scope, revoked membership, DTO minimization, both page-load binding states, all visible lifecycle states and immutable-receipt navigation. All 12 workspace typechecks and web lint pass. The root production build generates 94/94 Next.js pages/routes and the 18-module companion frontend (201.34 kB JavaScript, 64.02 kB gzip).
- A fresh disposable migration target retains exactly 116 ledger rows ending at 0116 and 136 public base tables. Its immediate rerun applied nothing and skipped all 116 unchanged. Frozen SHA-256 values remain 0113 `2431cf89e54705443eca1ca390aa082f6ecb51e0a6a5509769c1a4944e2c924f`, 0114 `36886f10b715a2ef0797a7d4dcdd19e1075b8f5c36ddeb51c41c23f23b31e320`, 0115 `a8a0acc7e43700a591ca75904dc2737ce7cac88bb418ad72719208ce06d793c6` and 0116 `3e756bccfde1d6b7836f6ddb71b3b0c4861d6c6afb1bfa87dfb5a20094b3c652`.
- Authenticated browser acceptance exercised historical no-command, an enabled revision-1 binding, the always-present commit-time race advisory, explicit reapproval, exact pending status, one real ingestion-worker completion, immutable preparation-receipt navigation and **Prepared—not activated** output. Desktop and 430-by-900 mobile views had no horizontal overflow, and current-session React error/warning logs were empty. Production-mode smoke returned health 200/version 1.26.0, redirected unauthenticated Smart Sources to login, kept development login disabled and honestly reported readiness 503 for absent deployment origin/identity/storage and worker freshness while database/migrations were ready.
- Native check, all three Rust tests and x64 NSIS packaging pass. The unsigned `Market Me Companion_1.26.0_x64-setup.exe` is 2,944,124 bytes with SHA-256 `df3abd96404acd821331ccaa9fbfc25d773d48d4f7bc8e4490665be3eeb17f67` and Authenticode `NotSigned`; it was not installed, signed or uploaded.
- Exact implementation commit `8d23c4bafc852ded456eef54b49af3f476a72f0c` passed feature-branch [GitHub Actions run 35045757711](https://github.com/GDS-G/Market-Me/actions/runs/35045757711) in 2 minutes 44 seconds. The clean Ubuntu job added 609 packages and audited 622 with zero vulnerabilities, passed native dependency smoke, applied all 116 migrations, generated Next types, passed all 12 workspace typechecks, lint, the same 2,020-test/118-file split, both production builds, cleanup and both dependency audits. No cloud-only source correction was required.
- The Google development parent and all five child tabs contain the 1.26 design, variables/lifetimes, build/recovery, security and feature-run evidence. Connector readback at revision `ANLCKQlb8GkLMYr3oAyJQScpH6Loyrxr59koVpH548zZxoceA6ouIwWJqfzXRHKWxdWGuQMBLnHknnPqlG9Qcs5yfob3FhYjbXQMw5zBHg` verifies each new heading/body-list role, the immediately following 1.25 section and the unchanged 30-tab title/order/parent topology. The skill's advisory file-backed trusted-read bridge rejected the Windows absolute-path convention, so verification used native connector structure/text reads; no rendered-PDF visual-layout claim is made. Reviewed documentation feature CI, `main` CI and public-release evidence are intentionally not claimed until observed.

The next planned slice is a side-effect-free, server-normalized Smart Source preparation plan preview. Reusable template persistence, ready-before-approval behavior and automatic finalization/activation remain later authority-bearing work.

## 1.25.0 - Source-bound draft preparation

**Status: final local gate, Google-document readback, exact feature/main Linux CI and reviewed-source publication are verified.** Migrations, integrated tests, typechecks, lint, frontend builds, authenticated browser acceptance, production smoke and Windows native/package checks pass from the exact 1.25 source. Local dependency audits were blocked before transmission by the execution environment's registry-disclosure safeguard, so no local audit result is claimed. Reviewed release commit [`d9ff270d15bfe143c839e17c007fc558658eb62b`](https://github.com/GDS-G/Market-Me/commit/d9ff270d15bfe143c839e17c007fc558658eb62b) passed [feature Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/35041096710) in 3 minutes 53 seconds, was fast-forwarded without divergence to `main`, and passed [main Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/35041414004) in 2 minutes 56 seconds with the same 2,015-test/118-file split, all 116 migrations, typecheck, lint, production builds and both zero-vulnerability audits. Google development-document publication readback is complete. This slice does not complete the whole application or authorize automatic publishing, activation or sending.

- Adds one mutable `smart_source_preparation_binding` per Smart Source for built-in `general_announcement@1`. The authenticated owner/admin/editor saver becomes the configured writer. Optional current published Brand, up to 20 unique ordered current published Audiences, optional published Destination, name/description, information depth, promotional strength and IANA timezone are validated. The binding `enabled` switch is independent from `smart_source.enabled`: pausing synchronization does not disable future explicit approval-triggered preparation.
- Adds normalized `smart_source_preparation_binding_audience` ordering and a durable `source_preparation_command` approval outbox in base migration `0115_source_bound_draft_preparation.sql`. Forward migration `0116_source_preparation_recovery_hardening.sql` permits safe disablement while identical saved references are stale, revalidates the full retained set before enablement, reserves source-command keys, linearizes approval with disablement, and hardens exact completion/recovery. Only a newly recorded non-null exact approval-pointer transition observed with an enabled binding enqueues. Migration/binding creation/enablement does not backfill, same-receipt approval replay does not duplicate, and binding changes after enqueue do not mutate/cancel the command.
- Each command immutably pins workspace/source/package/version, exact approval ID, review fingerprint, configured writer, binding revision and normalized configuration/binding snapshots. `expected_approval_id` is globally unique and `preparation_idempotency_key` equals command UUID. That UUID is a hidden internal namespace reserved to the exact processing command's writer, package/version, configuration, approval and fingerprint. Every explicit reapproval therefore may create a distinct command, while a byte-identical later approval cannot authorize an older unstarted command.
- The ingestion worker claims one command immediately before work with `FOR UPDATE SKIP LOCKED`, a bounded lease and incremented attempt fence. It overwrites JSON scope with dedicated relational columns and calls the existing `CampaignPreparationRepository.prepare()` with the command UUID and exact approval/fingerprint. A crash or retryable response/settlement loss after preparation commit reconciles the exact original receipt/Campaign. Lost attempts cannot settle replacements; deterministic invalid lineage dead-letters. Attempts 1–7 use 5, 10, 20, 40, 80, 160 and 320-second retry delays; an expired eighth attempt recovers an exact receipt or dead-letters without a ninth claim.
- Adds strict no-store GET/PUT binding API and Smart Source configuration/history UI. PUT requires configured Origin, current writer access, matching explicit workspace scope, `application/json` streaming-bounded to 32,768 bytes and an unknown-field-free schema. Browser models omit command UUID, writer/approval/fingerprint/idempotency/lease/snapshot authority. Content Package approval warns that every new explicit approval can queue another draft-only preparation; completed status links to the immutable preparation receipt.
- Preparation produces only a planning Campaign, an internally published immutable `draft_only` planning version, governed draft variants, a completed preparation receipt and minimized audit evidence. Internal planning-version publication is not external publication. The feature creates no Draft/Campaign approval, finalization, Campaign instance, workflow command, activation, schedule, publication action, credential access, provider call or send.
- Frozen migration `0115_source_bound_draft_preparation.sql` has SHA-256 `a8a0acc7e43700a591ca75904dc2737ce7cac88bb418ad72719208ce06d793c6`; forward hardening migration `0116_source_preparation_recovery_hardening.sql` has SHA-256 `3e756bccfde1d6b7836f6ddb71b3b0c4861d6c6afb1bfa87dfb5a20094b3c652`. A zero-to-116 replay completed in disposable `market_me_ci`, ending at ledger row 116/0116 with 136 public base tables. The immediate rerun applied nothing and skipped all 116 checksums unchanged. Both files are now immutable; corrections require migration 0117 or later.
- The full local suite passes **2,015 distinct TypeScript tests across 118 files without skips**: web 629/37 files, workflow-worker 68/5, companion protocol 4/1, connectors 166/10, database 958/48, domain 65/3, generation 29/4, ingestion 29/5, media 24/2 and workflows 43/3. Focused source-preparation gates pass 14/14 live database, 18/18 service and 95/95 web tests. All 12 workspace typechecks and web lint pass. The root production build generates 94/94 Next.js pages/routes and the 18-module companion frontend (201.34 kB JavaScript, 64.02 kB gzip).
- Authenticated development-browser acceptance began against the ordinary application database and correctly failed closed before migration because the new binding table was absent. After applying the frozen forward set through 0116, the page loaded. A disabled synthetic Smart Source displayed all future-only/no-backfill/durable-snapshot/no-send safety text; an enabled no-profile/no-Destination General Announcement binding saved at revision 1, a description edit saved at revision 2, and the source list displayed **Draft Prep Enabled** beside **Sync paused**. Both PUTs returned 200. No Content Package approval, command, Campaign, external action or provider call was created by this configuration-only check.
- Exact production-mode smoke started the built Next.js server in 168 ms. `/api/health` returned 200/`ok`/version 1.25.0, unauthenticated `/smart-sources` redirected 307 to `/login`, and production POST `/api/auth/dev-login` returned the expected 404 `development_login_disabled`. `/api/ready` returned an honest 503 for absent production origin/identity/storage/worker configuration while database connection and all 116 migrations were `ready`. The server was stopped after the checks.
- `cargo check`, all three Rust tests and x64 NSIS packaging pass. The local unsigned `apps/companion/src-tauri/target/release/bundle/nsis/Market Me Companion_1.25.0_x64-setup.exe` is **2,947,722 bytes**, SHA-256 `22b48b2e31006141b3eec0e43d6ce893b69e3337900fc4643d604379f7f0e726`, Authenticode `NotSigned`. It was not installed, signed or uploaded.
- Local `npm audit --omit=dev` and `npm audit` calls were blocked before registry transmission because the environment required separate authorization to disclose the dependency/workspace metadata payload. No local clean or vulnerable result is claimed. Reviewed feature run 35041096710 and identical main run 35041414004 independently executed both ordinary audit commands against the public source; each command reported zero vulnerabilities in both runs.
- Reviewed release commit `d9ff270d15bfe143c839e17c007fc558658eb62b` passed feature-branch run 35041096710 in 3 minutes 53 seconds and main run 35041414004 in 2 minutes 56 seconds after a divergence-free fast-forward. Each clean Ubuntu job installed 609 packages and audited 622 with zero vulnerabilities, loaded the native dependency set, applied all 116 migrations ending at 0116, passed the same 2,015-test/118-file package split, generated types, passed all 12 workspace typechecks and lint, generated 94/94 Next.js pages, built the 18-module companion, cleaned up and passed both audits. No cloud-only source correction was required.
- Updated the Google development parent and all five child tabs with the 1.25 architecture, complete field/collection/variable lifetimes, build/migration/native evidence, security/recovery limits and final feature/main CI/publication result. Native connector readback at revision `ANLCKQmzfCVcDNGdSlbTbUXQWoCm0pnY_n7GXOTc0FaIMRJJRe7-dBYI__3kfiv_1ZRIbHZ3URoo1gsv_UtkK4VHFT-c0zAj-au6nsR64A` verifies preserved heading/list roles, the immediately following 1.24 sections and the unchanged 30-tab title/order/parent topology. The skill's advisory file-backed trusted-read bridge rejected the Windows path convention before writing, so verification used native targeted reads plus complete connector topology/style readback; no rendered-PDF visual claim is made.
- Rollout is coordinated and forward-only after migration application: drain 1.24 approval and ingestion writers, back up/verify frozen 0113–0114, apply 0115 then additive 0116, deploy matching database/ingestion/worker/web code, confirm readiness reports 116/latest 0116, validate no-backfill plus exact disable/enable/approval ordering/enqueue/processing/recovery/status, then reopen. A code-only rollback leaves the additive triggers/tables active; stop new binding/approval traffic, retain commands and preparation history, and prefer a forward repair.

See [Source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md) for exact tables, fields, variables, states, API, roles and required acceptance; [Developer Guide](DEVELOPMENT.md) for build/rollout; and [Recovery](RECOVERY.md) for restore disposition. The exact reviewed release commit is published on `main`; hosted deployment remains a separate unperformed operation, and this release is not a whole-product completion claim.

## 1.24.0 - Exact Content Package review and approval

**Status: final local gate, Google documentation readback, exact feature/main Linux cloud CI and reviewed-source publication are verified.** Exact runtime source `aa6c1c25e84317f06193d59b9849e9c899ec50c7` passed clean installation, all **1,932 TypeScript tests across 113 files without skips**, migration replay, typecheck, lint, production builds and both audits. Reviewed release commit [`6ed85fb7d922d7f51b7d3304c4c536a28c7ebc7e`](https://github.com/GDS-G/Market-Me/commit/6ed85fb7d922d7f51b7d3304c4c536a28c7ebc7e) passed [feature-branch Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/35022144834) in 3 minutes 51 seconds, was fast-forwarded to `main`, and passed [main Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/35022617653) in 3 minutes 11 seconds with the same test split, all 114 migrations, typecheck, lint, production builds and clean audits. Authenticated development-browser acceptance, production smoke and native packaging passed separately. Package and native metadata are synchronized at 1.24.0. This slice does not complete the whole application or authorize automatic publishing, activation or sending.

- Added one coherent, bounded `ContentPackageReviewSnapshotV1` over exact package identity, ordered evidence/conflicts/assets, extraction, scan, accessibility, rights and raw metadata. Strict validation preserves six-digit UTC, bigint and lossless JSON values before domain-separated canonicalization and SHA-256 fingerprinting. A deterministic `effective-evidence-v1` evaluator fails closed on incomplete, contradictory, cyclic, oversized or lossy review state.
- Added mandatory numeric-version plus fingerprint preconditions for material evidence, conflict, accessibility and rights actions. Approval rechecks current approver authority and eligibility under locks, then atomically stores an immutable receipt, Learning Review proof, current pointer and minimized audit record. Same-version material changes invalidate only the current pointer; historical receipts, decisions and generated-work provenance remain retained. A fingerprint, status flag, receipt ID or browser role is never a bearer permission.
- Draft generation, General Announcement preparation and new Campaign finalization now require the exact current package approval and approved effective-evidence identities. Existing Draft approval, preview, account, publication, activation, runtime and provider authorization remain independent. Finalization may accept a newer receipt only for byte-identical reviewed content and retains the generation's original provenance.
- Frozen stop-the-world migration `0113_content_package_approval_receipts.sql` has SHA-256 `2431cf89e54705443eca1ca390aa082f6ecb51e0a6a5509769c1a4944e2c924f`. The committed-1.23 upgrade rehearsal reached 0113 with exact legacy bytes, provenance pins and completed recovery behavior preserved. Additive function-only migration `0114_learning_review_evidence_nullification_guard.sql`, SHA-256 `36886f10b715a2ef0797a7d4dcdd19e1075b8f5c36ddeb51c41c23f23b31e320`, rejects direct Learning Review evidence detachment while preserving a genuine source-refresh foreign-key action. Isolated QA applied and reran all 114 migrations with unchanged checksums and 133 public base tables.
- Exact runtime source `aa6c1c25e84317f06193d59b9849e9c899ec50c7` completed `npm ci` for 597 packages in 16.014 seconds and confirmed the clean install in 15.674 seconds, with zero reported vulnerabilities. The full sequential suite passed all 1,932 distinct TypeScript cases across 113 files without skips in 99.410 seconds: web 578/34 files, workflow-worker 68/5, companion protocol 4/1, connectors 166/10, database 944/47, domain 65/3, generation 29/4, ingestion 11/4, media 24/2 and workflows 43/3. All 12 workspace typechecks passed in 50.611 seconds and lint in 50.931 seconds. The 21.984-second root build passed: Next.js compiled in 4.6 seconds, checked TypeScript in 3.2 seconds and generated page data for 94/94 routes in 566 ms; companion Vite built 18 modules in 387 ms with a 201.34 kB JavaScript bundle (64.02 kB gzip). Full and production-only audits passed with zero vulnerabilities in 1.274 and 1.099 seconds.
- Reviewed release commit `6ed85fb7d922d7f51b7d3304c4c536a28c7ebc7e` passed feature-branch GitHub Actions run 35022144834 in 3 minutes 51 seconds and main run 35022617653 in 3 minutes 11 seconds after a divergence-free fast-forward. Each clean Ubuntu install added 608 packages and audited 621 with zero vulnerabilities; native dependency smoke passed. Both cloud databases applied all 114 migrations with no skips. The same 1,932-test/113-file package split passed, followed by generated types, all workspace typechecks, lint, a 94-page Next.js build, the 18-module companion build, cleanup and both zero-vulnerability audits. No cloud-only source correction was required.
- Final-review corrections isolate the governed-draft fixture in one marked disposable organization, refuse non-QA databases and serialize `seed`, `bridge` and `clean` with one reserved-session advisory lock; preserve the established Content Package v1 response while moving exact review hydration to its dedicated route; remove a duplicate original-image accessibility blocker; inherit approved or decorative alternative-text semantics from the captured original for derivative previews; close direct Learning Review evidence-pointer detachment; and clean every repository fixture user so the shared QA database returns exactly to baseline. Live seed/clean/repeat and non-QA rejection restored the exact baseline. Four focused web files passed 177 cases, including the 146-case route/path subset, plus web typecheck and lint; derivative rendering passed 16/16 plus web typecheck and focused lint; repositories integration passed 7/7 plus database typecheck; the combined 0114 approval/generation gate passed 39/39, its rights boundary passed 2/2 and readiness passed 11/11 plus database typecheck. The previously frozen exact-review race suite passed 17/17. These focused reruns corroborate the final integrated gate and are not additional distinct cases.
- The final migration rerun completed in 1.187 seconds with all 114 checksums unchanged and 133 public base tables. Strict cleanup compared a 193-row QA baseline before and after the complete gate: fingerprint `afd9f218f863865633cac65be978dffb` was identical, with zero fixture residue. This is an exact database-state comparison, not merely name-based cleanup.
- Authenticated development-browser acceptance covered all four queue states. A clean approval returned 201, exposed its immutable receipt, supported lookup and replayed the same idempotency key with 200 while retaining one receipt. Conflict resolution changed the fingerprint and effective/excluded evidence before the next 201 approval. Accessibility PATCH and restricted-rights PUT preserved their fail-closed blockers, and a legacy approval appeared only as unverified history. Desktop 1280x720 and mobile 390x844 layouts had no horizontal overflow; all observed review requests returned expected 2xx statuses, including 201, with zero console warnings or errors. Owned fixtures were cleaned and the exact QA baseline was restored.
- Exact-production smoke started the built Next.js server ready in 176 ms. The authenticated review and immutable receipt rendered; explicit **Load current review** hydration advanced `evaluatedAt`, proving a fresh evaluation rather than a stale page snapshot. `/api/health` returned 200 with version 1.24.0, an unauthenticated Content Package request redirected 307, and the production development-login route returned 404. The server was stopped and the QA baseline remained restored. This is local production-mode acceptance, not deployed OIDC or provider acceptance.
- Companion Vite production build passed with 18 modules and a 201.34 kB JavaScript bundle (64.02 kB gzip). `cargo check`, all three Rust tests and x64 NSIS packaging passed; binary/doc targets had zero failures. The local unsigned `apps/companion/src-tauri/target/release/bundle/nsis/Market Me Companion_1.24.0_x64-setup.exe` is 2,944,389 bytes, SHA-256 `93ef638a970a44e11361b4b23ad8a287816d0ebf9717af00124a86a6daa916e5`, Authenticode `NotSigned`. It was not installed, signed or uploaded.
- Updated the Google development parent and all five child tabs with the exact 1.24 architecture, variable/data lifetimes, build/rollout/recovery evidence, security boundaries, release artifact and final branch/main CI/publication record. Native connector readback at revision `ANLCKQl6Da1MFVjyEE0CCn57p63O9gUajIQntnGQnvHX4JfUROxANaSpDgmqn8VximQShf33TWLQ__qW8Wvom9tnPEFcmOyFR2b2HPvI2g` verifies one 1.24 heading and six list paragraphs in each tab, preserved list/body roles, the immediately following 1.23 sections, the explicit `{workspaceId}` review-route placeholder and the complete 30-tab hierarchy. All 23 conceptual children and older development history remain present. This is native connector text/style verification, not PDF visual-layout acceptance; the current Windows sandbox could not persist a fresh full trusted-read bridge artifact, so final verification used targeted native reads plus the earlier complete zero-control inventory.
- Rollout must stop and drain every old review, ingestion, Learning Review, package/asset, generation, preparation and finalization writer; apply 0113 then 0114; deploy matching 1.24 readers/writers/UI; validate exact review, mutation, approval, replay and consumer gates; then reopen writes. A code-only rollback to 1.23 after these migrations is unsafe. Preserve immutable decisions and external outcomes, keep writes closed on failure and prefer a forward repair. No new environment variable, provider permission or paid/live provider call is introduced.

See [Exact Content Package review](CONTENT_PACKAGE_REVIEW.md) for schema, limits, locks, variables and recovery, and [Developer Guide](DEVELOPMENT.md) for coordinated deployment. The exact reviewed release commit is published on `main`; hosted deployment remains a separate unperformed operation.

## 1.23.0 - Exact-reviewed-preview finalization (local preview)

**Status: verified local preview and Linux cloud CI; source published on main.** Reviewed source [`02d84a0531a01bb6a07f9d25d3292808b55189b3`](https://github.com/GDS-G/Market-Me/commit/02d84a0531a01bb6a07f9d25d3292808b55189b3) passed [Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/34438005045) in 3 minutes 44 seconds, including all **1,405 tests across 101 files without skips**, clean installation, 112 migrations, type/lint/frontend builds and both clean audits. Exact tested source was fast-forwarded to main. Metadata is synchronized at 1.23.0. This slice creates a protected executable draft from one reviewed prepared variant; it does not complete the whole application or automatically publish/activate/send.

- Added strict same-Campaign finalization, current writer/source/profile/approval checks, a coherent server-generated raw preview token, exact timing normalization, immutable completed retry receipts and one approval-required text-only official-API step. Supported providers are Discord, Slack and Mastodon. Ordered original profile pins remain unchanged; selecting one variant does not send all variants.
- Added v1 canonical JSON fingerprinting with exact Unicode/URL spelling, raw capability/UTM preservation, PostgreSQL numeric round-trip verification, six-digit UTC timestamps, bounded object traversal and strict account identity. Same-ID preview recreation changes the token; display/health labels do not. Tokens are change detectors, not authentication or approval.
- Protected provenance is mandatory at activation, pinned execution validation and locked new/retry publication admission. Advanced editing/replacement is denied. Database content and transaction-local admission fences block incompatible workers; previously accepted/in-flight/ambiguous history stays recoverable without resend. Corrected optional absent Discord guild handling and locking of the actual preview-owned tracked link.
- Migration `0112_campaign_finalizations.sql`, SHA-256 `f38620c1b9c61a2e8298b9a1eb70b2b7a37ddc453e1f73f15ef243575c479566`, applied cleanly to isolated QA123: 112 migrations, 131 public base tables. Checksum rerun skips all 112 unchanged. Application and older QA databases are untouched.
- Full local suite passes **1,405 distinct TypeScript cases across 101 files with no skips**: web 389/29 files, workflow-worker 68/5, companion protocol 4/1, connectors 166/10, database 606/40, domain 65/3, generation 29/4, ingestion 11/4, media 24/2 and workflows 43/3, including three actual Temporal history replays. New backend cases: finalization compiler 101, fingerprint primitive 134, raw loader 52, atomic finalizer 57, helper/control 23, raw database fences 21 and protected activation/admission 30. Subset reruns are not additional cases.
- All workspace typechecks and lint pass; full and production-only audits report zero vulnerabilities. Companion native check, all three Rust tests, frontend build (18 modules) and x64 NSIS packaging pass. The unsigned `Market Me Companion_1.23.0_x64-setup.exe` is 2,944,620 bytes; SHA-256 `4ddbf7f3560d3948d7c79522982443e5884cc6dcac35c76af1dd8ae5fa640782`; Authenticode `NotSigned`. Prior 1.21/1.22 artifacts and hashes are unchanged. No installation, signing or upload occurred.
- Authenticated development browser acceptance selected one of three explicit previews with Unicode and retained `.654321` preview/`.123456` capability timestamps. An expired-window POST returned 409 with no executable version/receipt; reload, lookup 404 and repeat 409 retained the original request. Explicit reset/review then created the same Campaign's v2 with exact `.125`/`.999` UTC window bounds. Direct Publish returned 200 without PATCH; activation stayed separately unchecked/disabled. Viewer result/editor had no writer controls, and the finalizer URL rediscovered the existing receipt. Before scoped cleanup: one Campaign, two versions, zero runs/commands/publications. No browser errors; unrelated extension warnings only. Transport-loss injection is a helper-test result, not a browser claim. Owned fixture/session/tab/server were cleaned.
- Final source builds successfully with Next.js 16.3.4 (94 generated pages) and companion Vite frontend (18 modules). The final action-control styling correction passed all 389 web cases, typecheck and lint before its separate source commit. It changes layout only, not finalization authority.
- Exact-production smoke passes: root, new finalizer and final-result routes redirect 307 to login; login returns 200 without development sign-in or redirect loops. All observed assets (one CSS and eight JavaScript bundles) return 200, correct MIME and nonempty bodies. Browser/server errors are absent. Readiness honestly returns 503 for missing production identity/storage/workers and a non-HTTPS test origin, while database connection and schema112 are ready. No login, provider call or database write occurred; owned tab/server were closed and loopback3119 is clear. This is not deployed OIDC acceptance.
- Updated the Google development parent and all five child tabs with current architecture, variable/data lifetimes, migration/operations, authority limits and exact source/CI links. Native readback verifies all six new heading/list sections, both source links and preserved typography, while retaining the 30-tab hierarchy, every conceptual tab and all older development history. This is native text/style verification, not PDF visual-layout acceptance.

See [Finalization contracts](CAMPAIGN_FINALIZATION.md) for the programmer model and [Developer Guide](DEVELOPMENT.md) for coordinated rollout. No installer upload, hosted deployment, live-provider acceptance or paid calls occurred.

## 1.22.0 - Review-first campaign preparation (local preview)

**Status: verified local preview and Linux cloud CI; source published on main.** Reviewed source [`f174493cbb7621e367a81caddd2261907a001377`](https://github.com/GDS-G/Market-Me/commit/f174493cbb7621e367a81caddd2261907a001377) passed [Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/34435564818) in 3 minutes 30 seconds, with all **864 tests across 90 files without skips**, clean installation, 111 migrations, type/lint/frontend builds and both clean audits. The exact tested source was fast-forwarded to main. Metadata is synchronized at 1.22.0. This release creates reviewed planning material, not an executable publishing plan or a complete beginner workflow. No production deployment, live provider acceptance or signed distribution is implied.

- A strict versioned General Announcement compiler emits an ordinary `draft_only` Campaign with one manual review step and explicit package/profile/copy controls. No account, attachment, arbitrary graph, context or approval override is inferred. Defaults are contextual/informational/UTC; audience selection order is preserved.
- Atomic preparation rechecks current writer authority and exact approved package revision, current published Brand/Audience pins and optional published Destination under locks, then creates the planning Campaign, governed variants, immutable completed receipt and audit. Identical retries return the same original IDs after later source/profile changes; changed payload under a tenant/key conflicts. There are no workflow instances, commands, publications or approvals from preparation.
- New transaction-only Campaign/draft helpers retain existing validation and two-pass immutable evidence insertion. The preparation path uses exact created IDs and never chains public transactions or post-commit workspace-wide hydration. Its result remains historical even if live draft versions change later.
- Live testing found SQL-null profile values entering ordinal sorting and masking another audience's promotional ceiling. The shared domain resolver now treats null/undefined as absent while preserving every real least-permissive limit. Both starter and advanced authoring paths reject violations; four added domain cases cover ordering, null defaults and custom controls.
- A deterministic preview race reproduced old 500-character rendering receiving a newer 10-character capability timestamp and remaining eligible. The fix captures raw capability JSON and its full-precision timestamp under a connection lock acquired before draft/version locks. Read/picker/activation/publication eligibility requires exact stored/current JSONB and provider/timestamp agreement. Historical normalized/mismatched snapshots now fail closed as stale and must be recreated/reviewed; no old row, copy or approval is deleted. These are capability-consistency checks, not immutability of the preview's ID/content.
- Migration `0111_campaign_preparations.sql`, SHA-256 `c4c51566c1cbe394766c9f6d677e12a97f43171ea281e7bb8fa895e96a42323a`, applied cleanly to isolated QA122 with all 111 migrations and 130 public base tables. Readiness expects exactly 111 and the latest filename. No new dependency resolution, environment variable or provider permission is introduced.
- The complete local suite passes **864 distinct TypeScript tests across 90 files without skips**: web 274, workflow-worker 60, companion protocol 4, connectors 166, database 188, domain 65, generation 29, ingestion 11, media 24 and workflows 43. All workspace typechecks and the 111-migration checksum rerun pass. Included cases cover 34 compiler tests, 38 live preparation tests (including ten lock races), seven new preview regressions, 86 new web tests and three actual Temporal history replays. Subset reruns and assertions are not additional cases.
- Authenticated development-browser acceptance passed: one 201 response created one planning Campaign and two ordered variants, with no inferred Brand/Destination, connections, runs or publications. Source refresh to revision 2 retained both original revision-1 evidence traces. Reload restored the exact frozen request/key; explicit lookup and identical POST retry returned the original receipt without duplication. Prepare another cleared only the completed recovery copy, Campaign cards rediscovered the receipt, and a viewer received no write controls. Readiness was 200 on schema 111; browser errors were empty. Only the owned synthetic fixture/server/tab were cleaned.
- Full and production-only dependency audits report zero vulnerabilities. The exact production build passes Next.js generation of 93 pages and the 18-module companion frontend. Production browser smoke passed protected-root/preparation/receipt redirects, login, one CSS and eight JavaScript assets, and empty browser/server errors; development login was unavailable. Production readiness correctly remained 503 without production OIDC/storage/workers and with an HTTP origin, while database connectivity/schema 111 checks were ready. The loopback server and owned tab were closed without database writes. This is not deployed authentication acceptance.
- The Google development parent and five children contain the 1.22 architecture, fields/collections, build/recovery rules, compatibility limits and exact source/CI evidence. Native readback verifies new headings/lists/inherited typography and source links, and preserves every prior development section, all conceptual content and the complete 30-tab hierarchy. This is native text/style verification, not PDF visual-layout acceptance.
- Native checks, three Rust tests, companion frontend and NSIS packaging pass. Local unsigned `Market Me Companion_1.22.0_x64-setup.exe` is **2,947,354 bytes**, SHA-256 `a10942d2a2c69b9db1041fda1a5705dbbe4a36a52ba2dd5bd8dd4e54c891b6f7`. Nothing was installed, signed or uploaded; the 1.21 installer is unchanged.

Rollback retains preparation receipts, planning ancestors and generated drafts; disable the new entry point rather than deleting retry evidence. There is no receipt expiry/deletion API or down-migration. Current role checks remain required for replay. Destination snapshots are unversioned historical context, not send authority. The future same-Campaign executable-plan finalizer must reject existing editable drafts and compare exact mutable preview content, not only its ID. Full contracts and variable lifetimes: [Campaign preparation](CAMPAIGN_PREPARATION.md).

## 1.21.0 - Immutable historical draft evidence (local preview)

**Status: verified local preview and Linux cloud CI; source published on main.** Evidence source `692a136f5d3e3c395fcc308c668133edbeec2f71` plus test-only acceptance correction `2640b75ee6635f0c020259bf864d81eaccf8bdde` passed [Linux CI](https://github.com/GDS-G/Market-Me/actions/runs/34433881287) in 3 minutes 19 seconds, including all **695 tests across 83 files without skips**, clean installation, migrations, type/lint/frontend builds and both clean audits. The exact tested source was fast-forwarded to main. Metadata is synchronized at 1.21.0. The review-first campaign starter remains planned in this release; it repairs the historical-evidence prerequisite. No hosted deployment, live provider acceptance, signed installer distribution or whole-product completion is implied.

- Reproduced real package refresh deleting older draft claim links through a live-evidence cascade while approved copy and generation snapshots survived. Migration 0110 retains the existing historical UUID links independently of replaceable `evidence_item` rows.
- New SQL proof validation checks exact workspace/campaign/package/generation/version lineage, complete nonnegative contiguous fact order, unique captured IDs and matching factual text/provenance. New/revised links require that proof; existing link identity cannot be changed. Writers insert all claims before links in one transaction.
- Conservative backfill restores only entirely unlinked claims with complete immutable proof. An independent 109-to-110 rehearsal generated eight real two-fact historical variants with identical text/distinct IDs, exercised actual source refresh, then applied the migration. Three provable links were restored; malformed/ambiguous histories received none, and a contradictory surviving reference remained unchanged rather than being augmented. Rerun skipped all 110 with matching checksums.
- Ordinary and AI revisions retain every fact with left joins and fail closed for absent or contradictory evidence. AI context validates proof before creating a paid-provider intent and again on apply. Generation's shared package lock prevents an earlier approval/revision from being combined with refreshed evidence. No old approval is transferred to a new package or draft.
- Draft trace distinguishes captured references, unavailable history and presentation-only calls to action. Missing/duplicate snapshot IDs and factual text mismatches are marked explicitly; no current-evidence lookup or guessed remapping substitutes for history.
- Migration `0110_immutable_draft_claim_evidence.sql`; SHA-256 `249e7ab29f1158e9092548530b731c8648c9e9e6c3581d8968a02c6bc536cdff`. Fresh QA replay has 110 migrations and 129 public base tables. Readiness expects exactly 110 and the latest filename; the file-alignment regression remains in place.
- Final integrated run passed **693 distinct TypeScript tests across 83 files without skips**: web 188, workflow-worker 60, companion protocol 4, connectors 166, database 109, domain 61, generation 29, ingestion 11, media 24 and workflows 41. All 110 migration checksums passed on rerun. Full workspace typecheck, web lint, 91-page production build, companion frontend/native packaging and both zero-vulnerability audits pass. Counts are distinct test cases, never summed assertions or subset reruns.
- Initial cloud run [34433298420](https://github.com/GDS-G/Market-Me/actions/runs/34433298420) exposed a frozen 1.19 startup reporting race in the legacy pause test: an early buffered pause preserves the execution-blocking flag but startup can overwrite its status label with `active`. The steady-state test now waits for actual initial/paused persistence. Two deterministic `signalWithStart` cases prove legacy blocking and correct current-workflow `paused` reporting with zero dispatch through time advance and cancellation. The final local workflow suite passes 43/43, including three actual replays and frozen-source hash guards; production workflow source is unchanged. The clean-cloud suite confirms 695 distinct cases with workflows 43 and all other package counts unchanged.
- Local unsigned Windows artifact: `Market Me Companion_1.21.0_x64-setup.exe`, **2,944,521 bytes**, SHA-256 `7edc5421ba05be6ecff8b282ad4ee1f353034b707c19e0c46ebf18b707052421`. Three Rust tests passed. It was not installed, signed or uploaded; the 1.20 installer is unchanged.
- Final browser acceptance passed. Authenticated development fixture retained the original approved draft v1 text/IDs/source-revision trace after package v2 refresh, displayed a warning for an intentionally unlinked legacy fact, kept its matched sibling's trace and reserved presentation-only labeling for CTA. Readiness returned 200 with schema 110. Exact production build smoke passed one protected-root redirect, CSS plus eight JavaScript bundles and clean console/server output. Production readiness correctly remained 503 without production OIDC/storage/workers, while its migration check was ready. All owned QA tabs/servers and only the synthetic fixture were cleaned; this is not deployed authentication acceptance.
- Synchronized the Google development parent and all five child tabs with engineering contracts, migration/variable lifetimes, rollout/security limits and exact source/CI links. Native readback confirms all six heading/list sections, inherited typography and both source links. The 30-tab hierarchy and every conceptual tab remain unchanged. This is native text/style verification, not PDF visual-layout acceptance.

Rollout requires a coordinated writer pause: stop/drain ingestion, generation, ordinary revision and AI revision/apply writers; migrate 0110; deploy matching code; resume. Old interleaved claim/link writers can fail the new trigger, while new revision code requires the SQL helper. Mixed-version operation is not supported. Keep snapshots/links and do not restore the live-evidence cascade during rollback. The independent Temporal compatibility requirements from 1.20 still apply.

Limits: unprovable historical links cannot be recovered automatically; snapshot and parent metadata immutability relies on application/DB-role controls, not a universal privileged-database tamper barrier. This does not add templates, automatic source triggers, publishing authority or provider calls. See [Evidence retention](EVIDENCE_RETENTION.md) and the next [Campaign preparation plan](CAMPAIGN_PREPARATION_PLAN.md).

## 1.20.0 - Bounded request-start scheduling (local preview)

**Status: verified local preview and Linux cloud CI; source published.** Reviewed source `d38ae585aecb02cf8ea7fbf1916648c41e1a5995` passed [Linux cloud CI](https://github.com/GDS-G/Market-Me/actions/runs/34432169675) in 3 minutes 7 seconds, including all 682 tests without skips, clean installation, migrations, native dependency smoke, type/lint/frontend builds and both clean audits. Package/native metadata is synchronized at 1.20.0. This is not whole-product completion, hosted deployment or signed desktop distribution. Historical release records below remain unchanged in scope; see [Implementation status](IMPLEMENTATION_STATUS.md) for the full gap map.
...
- Updated the parent Google development tab and all five child tabs with the current design, variable/state contracts, migration/build evidence, security limits and source/CI links. Native readback verified all six sections, heading/list roles, inherited typography and exact links; the complete 30-tab hierarchy and all conceptual tabs are unchanged. This text-only update used native connector readback; no PDF visual-layout claim is made.

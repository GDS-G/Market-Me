...
## 1.23.0 - Exact-reviewed-preview finalization (preview under verification)

**Status: full local regression and native checks pass; browser/build/cloud release gates pending.** Metadata is synchronized at 1.23.0. The last published baseline remains 1.22. This slice creates a protected executable draft from one reviewed prepared variant; it does not complete the whole application or automatically publish/activate/send.

...
Status: implemented, verified and published on `main` for Release 1.24. Migration 0113 is frozen at SHA-256 `2431cf89e54705443eca1ca390aa082f6ecb51e0a6a5509769c1a4944e2c924f`; additive migration 0114 is frozen at SHA-256 `36886f10b715a2ef0797a7d4dcdd19e1075b8f5c36ddeb51c41c23f23b31e320`. The committed-1.23 upgrade rehearsal, isolated live application/rerun through 0114, final full local workspace/browser/native gates, Google development-document readback and exact feature/main Linux cloud runs passed. Reviewed release commit `6ed85fb7d922d7f51b7d3304c4c536a28c7ebc7e` was fast-forwarded to `main`. This document describes the current source contract, not a production-readiness or full-product-completion claim.
...
An immutable old approval remains readable after content changes. Replaying that receipt must never make changed content approved again. Existing generation and publication history is not rewritten to current source facts. See [evidence retention](EVIDENCE_RETENTION.md), [preparation](CAMPAIGN_PREPARATION.md), [source-bound preparation](SOURCE_BOUND_DRAFT_PREPARATION.md) and [finalization](CAMPAIGN_FINALIZATION.md) for the adjacent immutable contracts.
...
### Migrations 0113–0114: tables, columns and history guard
...
`0114_learning_review_evidence_nullification_guard.sql` is an additive, function-only hardening migration. It does not add a table, column or canonical contract. It replaces `preserve_proved_learning_review()` so a retained decision's live evidence pointer may become null only after its old parent evidence row is no longer present—the state seen during the existing foreign-key `ON DELETE SET NULL` source-refresh action. Direct nullification while the parent survives and every retarget/addition remain rejected. Do not alter frozen 0113 to incorporate this fix.
...
- `learning_review_proved_history_guard` refuses changes to every new or legacy decision row. Under 0114 it permits only non-null `evidence_item_id` becoming null after the old parent evidence row is no longer visible, which is the state produced by the existing foreign-key `ON DELETE SET NULL` source refresh. Direct `SET evidence_item_id = NULL` while that parent survives, null-to-ID attachment and ID retargeting all fail. Immutable decision JSON retains original evidence for proved rows.
...
`CampaignPreparationRepository.prepare(input, key, actor, {expectedReviewFingerprint, expectedApprovalId?})` retains General Announcement compiler v1 canonical configuration unchanged. New requests require the exact token separately; durable automatic commands also pin the exact receipt ID. The preparation reference snapshot adds `contentPackage.approvalId` and `contentPackage.reviewFingerprint` to its existing ID/title/version. Its exact generation pin and all initial variant IDs remain atomic. Completed new-format replay requires the same canonical compiler settings and original review token and, when supplied, the same approval ID, then returns the original receipt without reevaluating changed source/profile content. Completed legacy receipts without these new fields retain their prior recovery behavior; they are not upgraded to proof-bearing receipts. The browser distinguishes its new saved-attempt format from legacy recovery-only attempts.

Release 1.25 [source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md) reuses that same repository through a durable approval-triggered command. Only a newly recorded exact approval transition while an enabled Smart Source binding exists enqueues one command; approval replay and pre-existing approved packages do not backfill. Approval/disable races follow the binding lock's commit order. The command is pinned to the triggering approval ID, package version, exact configuration and review fingerprint, so a later byte-identical reapproval cannot authorize the older command; each explicit reapproval may enqueue a separate preparation. Its hidden UUID is reserved to the exact processing command and cannot be preempted by manual preparation. It still creates only an internally published immutable `draft_only` planning version and governed drafts. That internal planning transition is not external publication and grants no Draft approval, finalization, activation, workflow, scheduling, publication, provider or credential authority.
...
Migration 0113 is a **stop-the-world, forward-only writer migration**, not a rolling mixed-writer upgrade. Drain/stop 1.23 web mutation traffic and all ingestion, Learning Review, generation, preparation and finalization writers; apply reviewed migrations 0113 and 0114 in order; deploy the matching 1.24 writers/readers/UI; validate readiness and representative exact-review/new-work/recovery paths; only then reopen writes. Include background workers and queued work that can write these tables, not only the visible web server. The frozen 0113 checksum is `2431cf89e54705443eca1ca390aa082f6ecb51e0a6a5509769c1a4944e2c924f`; the frozen 0114 checksum is `36886f10b715a2ef0797a7d4dcdd19e1075b8f5c36ddeb51c41c23f23b31e320`. Isolated QA verified exactly 114 applied migrations ending at 0114, the unchanged 133 public base tables and a second runner pass that skipped all 114 unchanged.

Post-migration old Learning Review writers omit required immutable proof; old generation writers omit receipt provenance; old preparation/finalization writers cannot create new unproved lineage. A receipt writer also needs the exact transaction-local admission marker. They fail closed. This prevents accidental old-format acceptance, but intentional failure is not a zero-downtime compatibility promise. A code-only rollback to 1.23 while retaining 0113/0114 is unsafe for ordinary writes. Do not drop guards, null proof or evidence pointers, edit checksums or fabricate historical approvals to make it work. Prefer a forward repair; any complete database restore must be a coordinated recovery that accounts for all newer decisions and external publication outcomes.
...
Recorded development checks for this slice: 146 pure fingerprint cases, 34 effective-evidence cases, 78 mutation-helper cases, and 134 unchanged exact-preview fingerprint cases passed (392 distinct cases in that focused run); database TypeScript checking passed. Frozen 0113 passed transactional rollback probes for admission, immutable-history bypasses, legacy-row protection, workspace/organization cleanup and 10,000-evidence/2,000-conflict upper bounds. A genuine committed-1.23 database upgraded through 0113 successfully with exact legacy byte/provenance pins and completed recovery behavior preserved. Isolated QA subsequently applied 0114 for exactly 114 migrations in 133 public base tables, recorded both frozen checksums, and reran with all 114 skipped unchanged. The live approval/generation SQL suite passed 37/37, including direct evidence-detachment rejection and genuine source-refresh foreign-key detachment with the decision/proof/receipt history retained. The database-synchronized rights-boundary suite passed 2/2: future `validFrom` and `expiresAt <= now` inputs left rights, scope, package-status/current-pointer and rights-audit state unchanged; later time-only expiry made `currentApprovalValid` false while the stored approved status, receipt pointer and fingerprint remained unchanged, then blocked direct generation and Campaign preparation without consumer or audit writes. The live exact-review suite passed 17/17, including both root-lock orderings and membership revocation, with zero fixture users/packages or tagged backends left; repositories integration passed 7/7. The browser helper rejected an unknown command before database configuration, and a live PostgreSQL probe placed concurrent `seed` and `clean` processes behind one held fixture lock: both showed advisory waits, the second queued behind the first, no marker row appeared while blocked, and the error path released the lock for its successor. A subsequent successful seed/clean restored the exact non-fixture baseline with no lock session left. This is point-in-time pre-release evidence, not the final integrated release count. Mutation coverage includes 39 lossless timestamp cases, 36 human-review-state guards and three overlap-admission cases. Final full workspace, browser, cloud CI and native release acceptance is not asserted here yet.
...
- `content-package-approval-generation.integration.test.ts`: 37 executed live consumer/SQL guard cases require exact new generation evidence/order, receipt lineage, immutable proof/update/delete behavior, old-writer rejection, legacy completed replay, changed package rejection, identical-content reapproval, rejected direct evidence detachment and allowed real source-refresh foreign-key detachment.
- `content-package-rights-expiry.integration.test.ts`: two executed live cases take their boundary values and polling predicate from PostgreSQL time. They reject future-start/equal-now rights without rights, scope, package-status/current-pointer or rights-audit changes, then prove time-only `currentApprovalValid` invalidation with unchanged stored approval identity and zero-write generation/preparation rejection after expiry.
...
npm run test --workspace @market-me/database -- src/content-package-approval-generation.integration.test.ts src/content-package-rights-expiry.integration.test.ts
```

...
- Campaign form timestamps are **UTC inputs** even if a campaign has another presentation timezone. `2026-09-15T15:30:42.125Z` must survive an unchanged edit/save as that same instant. Preserve `preferredWindowStart`, `preferredWindowEnd`, condition JSON, optionality, execution methods, outputs, retry count, and timeout during ordinary editing. Saved recurring/window/conditional/evergreen fields are plans; unsupported execution is rejected rather than reinterpreted.

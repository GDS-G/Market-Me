...
Live acceptance fixtures use the exact `QA ... 0.13` names in a dedicated `QA Governed Drafts` workspace owned by the synthetic development user. The helper refuses any database whose URL path does not match `market_me_qa_*`. Run `$env:DATABASE_URL=...; npm run qa:governed-drafts -- seed`, select that workspace for browser checks, then run `npm run qa:governed-drafts -- clean`. Cleanup verifies the fixed organization/workspace markers, sole synthetic-user ownership and single-workspace boundary before deleting the complete disposable organization aggregate. It never deletes proof-bearing Draft, approval or Learning Review rows individually while their workspace survives.

...
## Release 1.25 source-bound draft-preparation candidate

Migration `0115_source_bound_draft_preparation.sql` is the additive base schema for the Release 1.25 candidate. It creates `smart_source_preparation_binding`, ordered `smart_source_preparation_binding_audience`, durable `source_preparation_command`, validation/immutability functions, and an `AFTER UPDATE OF current_approval_id` enqueue trigger. Forward migration `0116_source_preparation_recovery_hardening.sql` adds safe stale-reference disable/re-enable semantics, same-transaction parent-revision admission for Audience children, deferred enabled-Audience validation, source-command UUID reservation, approval/first-binding/disable commit ordering, strict due/lease/eight-attempt claim guards and exact receipt/configuration recovery. Candidate readiness expects exactly **116** migrations ending at 0116. Migration 0115 is frozen at SHA-256 `a8a0acc7e43700a591ca75904dc2737ce7cac88bb418ad72719208ce06d793c6`; 0116 is frozen at `3e756bccfde1d6b7836f6ddb71b3b0c4861d6c6afb1bfa87dfb5a20094b3c652`. A zero-to-116 disposable replay created 136 public base tables and the rerun skipped all 116 checksums unchanged. Never edit either file—use migration 0117 or later.

The implementation map is:

- `packages/database/src/source-preparation-repository.ts`: binding normalization/CRUD, stable reference/role locks, membership-scoped status reads, one-statement `SKIP LOCKED` claims, attempt-fenced completion/failure, retry scheduling and minimized audits.
- `packages/database/migrations/0115_source_bound_draft_preparation.sql`: one-per-source binding, ordered audiences, immutable command snapshot/state guards, exact-approval uniqueness and transactional enqueue with no backfill.
- `packages/database/migrations/0116_source_preparation_recovery_hardening.sql`: forward-only reference-disable/re-enable rules, deferred enabled-Audience check, command-key reservation, commit-order-safe approval enqueue and exact capped recovery/completion guards.
- `packages/ingestion/src/source-preparation-service.ts`: one-at-a-time claim/process loop, relational identity override, exact approval/fingerprint handoff to `CampaignPreparationRepository.prepare()`, safe error classification, settlement and fixed counters.
- `apps/worker/src/index.ts`: construction in the existing ingestion process and an independently caught source-preparation sweep. A command-store failure logs only `source_preparation_store_unavailable` and does not prevent later alert work.
- `apps/web/src/app/api/v1/smart-sources/[id]/preparation-binding/route.ts`: no-store GET/PUT binding API. GET returns the minimized binding only; source pages separately query minimized recent command summaries. PUT requires exact Origin/current writer access and accepts no actor, approval, command or result authority fields.
- `apps/web/src/components/source-preparation-binding-*`, Smart Source list/edit pages and Content Package review actions: binding form, ordered-audience controls, source/binding independence notice, durable command history/receipt links and explicit reapproval warning.

Use `SourcePreparationRepository.saveSourcePreparationBinding(input, actorUserId)` for the complete save. The authenticated owner/admin/editor becomes `writer_user_id`; database guards require it to match `created_by`/`updated_by`. A source has at most one binding and every save/toggle increments `revision`; existing saves require the browser's exact `expectedRevision` and fail closed on a stale value. Audience child mutation additionally requires transaction-local admission plus parent `xmin = pg_current_xact_id()`, proving the authenticated repository revised that parent in the same transaction rather than merely copying visible admission fields. Current published Brand/Audience versions and optional published Destination are validated for new, changed or enabled reference sets and rechecked during preparation. A disabled save may retain exactly the same reference IDs after they become stale so an operator can stop enqueue without first repairing them; it may not introduce or replace a stale reference, and re-enable proactively plus defer-validates the complete retained set. Do not add a browser-selectable writer, infer the approver as writer, or couple `binding.enabled` to `smart_source.enabled`. Pausing synchronization stops intake only. Disabling/editing applies to future exact approvals and must never update or cancel queued command snapshots.

Only a new non-null `content_package.current_approval_id` transition while the binding is enabled enqueues. Migration application, binding save/enable, existing approvals and replay of the same receipt enqueue nothing. Binding creation and approval take the same per-source transaction advisory key, so approval-before-first-binding and first-binding-before-approval have an unambiguous commit order. The 0116 trigger also holds the enabled binding `FOR SHARE` through approval commit: disable-first suppresses enqueue, while approval-first durably queues and makes disable wait. Every explicit reapproval has a new approval ID and may enqueue a new command even if its fingerprint is byte-identical. The command pins both values so an older unstarted command fails closed if a later approval supersedes it. Do not weaken the globally unique `expected_approval_id`, `preparation_idempotency_key = id`, immutable identity/snapshot fields or terminal-state guard.

Treat the command UUID as an internal reserved idempotency namespace. It is omitted from browser DTOs. Both `CampaignPreparationRepository.prepare()` and the 0116 `campaign_preparation` insert trigger permit a matching key only for a currently `processing` command with exact writer, package/version, `configuration_snapshot`, approval ID and fingerprint, while holding the command `FOR SHARE`. A manual caller may use its own UUID but cannot preempt, replay with altered configuration or adopt a source command's key.

The worker claims immediately before each preparation so later work does not age under a preclaimed lease. `SourcePreparationClaimBatch` is `{commands, recovered, deadLettered}`; its cleanup counts plus claimed commands consume the public batch limit, and the service's private `handled` counter keeps cleanup-only polls bounded. Returned/logged counters are `claimed`, `completed`, `retried`, `deadLettered`, `lost` and `settlementFailed`. `attempt_count` is the settlement fence: completion `false` or failure `undefined` means ownership was lost. Current-attempt completion with invalid receipt/Campaign/package/configuration/writer/approval lineage raises `23514`; deterministic failures are terminal. Retryable database/unknown failures after attempts 1–7 wait exactly 5, 10, 20, 40, 80, 160 and 320 seconds. `SOURCE_PREPARATION_MAX_ATTEMPTS = 8` applies to both settlement and claims: there is no ninth claim, and database guards reject future claim time, a lease beyond one hour and `failed` at attempt eight. Cap cleanup locks first with `FOR UPDATE SKIP LOCKED`; an in-flight receipt's `FOR SHARE` makes it skip until the receipt transaction settles. Before a retryable failure is stored—or an expired eighth attempt is terminalized—the repository reconciles an exact already-committed receipt to completed; only an exhausted row without that proof becomes `attempt_limit_exhausted`. If settlement storage itself is unavailable, leave the row processing for the same lease-expiry recovery. Never persist or log raw exception text.

Configure the existing ingestion worker with these non-secret variables:

| Variable | Default | Accepted operator range | Meaning |
| --- | ---: | ---: | --- |
| `SOURCE_PREPARATION_BATCH_SIZE` | `10` | integer `1..100` | Maximum claimed or cap-cleanup items handled in one ingestion sweep; the service still claims each command immediately before its work. |
| `SOURCE_PREPARATION_LEASE_SECONDS` | `300` | integer `30..3600` | Processing lease; choose longer than ordinary preparation time. UUID replay and attempt fencing make expiry recoverable. |
...
```powershell
npm run db:migrate
npm test --workspace=@market-me/ingestion -- src/source-preparation-service.test.ts
npm test --workspace=@market-me/database -- src/source-preparation-repository.integration.test.ts
npm test --workspace=@market-me/web -- src/components/source-preparation-binding.test.ts src/server/source-preparation-routes.test.ts src/server/source-preparation-pages.test.ts
npm run typecheck --workspace=@market-me/database
npm run typecheck --workspace=@market-me/ingestion
npm run typecheck --workspace=@market-me/worker
npm run typecheck --workspace=@market-me/web
```

The exact local candidate passes the frozen 116-migration replay/checksum gate, 14 live repository cases, 18 service cases, 95 focused web cases, the complete 2,015-test/118-file workspace suite, all 12 workspace typechecks, web lint, both frontend builds, authenticated browser acceptance, production smoke and Windows native/package checks. Google development-document synchronization and connector readback are complete with all 30 tabs preserved. Local npm audit commands remain unverified because the execution environment blocked registry disclosure before transmission. Implementation commit `48c975157cdd52abf29c9c606ea80e98efc6cec5` passed feature run 35040381880 with all cloud gates and both zero-vulnerability audits; the evidence/final reviewed commit, main cloud CI and source publication remain pending. Exact package totals, build results, Google revision and installer hash are maintained in [Releases](RELEASES.md).

Roll out 1.25 with coordinated writers: drain 1.24 web approval mutations and ingestion workers, back up PostgreSQL, verify frozen 0113/0114, apply 0115 and then 0116 once, deploy matching database/ingestion/worker/web code, confirm readiness reports 116/latest 0116, and validate no-backfill/save/disable/re-enable/approval-order/claim/reconciliation/completion/status behavior before reopening writes. An old web process can still trigger the database outbox for an already configured binding, while an old ingestion worker cannot consume it; do not intentionally operate mixed versions. A code-only rollback leaves both additive migrations and their triggers active. Stop binding creation and approval traffic, retain commands/receipts/planning history, and prefer a forward repair. Never delete/rewrite a completed or dead-letter command to retry; correct current configuration and make a new explicit approval only when new work is intended.

Preparation internally publishes an immutable `draft_only` planning version so generated drafts have a stable Campaign-version ancestor. In documentation, UI and logs, always distinguish that database transition from **external publication**. Source-bound preparation creates no Draft/Campaign approval, finalization, Campaign instance, workflow command, activation, schedule, publication action, credential access, provider call or send. See [Source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md), [Campaign preparation](CAMPAIGN_PREPARATION.md), [Security](SECURITY.md) and [Recovery](RECOVERY.md).

## Current implementation: 1.24 exact Content Package review

Migration `0113_content_package_approval_receipts.sql` is frozen at SHA-256 `2431cf89e54705443eca1ca390aa082f6ecb51e0a6a5509769c1a4944e2c924f`. Additive, function-only migration `0114_learning_review_evidence_nullification_guard.sql` is frozen at SHA-256 `36886f10b715a2ef0797a7d4dcdd19e1075b8f5c36ddeb51c41c23f23b31e320`; it replaces only the Learning Review immutability function so direct evidence detachment fails while a genuine foreign-key `ON DELETE SET NULL` source refresh still succeeds. The committed 1.23-to-1.24 rehearsal reached frozen 0113. Isolated QA subsequently applied exactly 114 migrations ending at 0114 into the same 133 public base tables and reran with all 114 checksums skipped unchanged. The final local gate passed clean installation, all 1,932 TypeScript tests across 113 files without skips, migration replay, typecheck, lint, production builds, both audits, browser acceptance, production smoke and native packaging. Reviewed release commit `6ed85fb7d922d7f51b7d3304c4c536a28c7ebc7e` passed exact feature and main Linux CI with the same 1,932-test split, all 114 migrations and clean build/audit gates before and after its fast-forward to `main`. The Google development parent and all five child tabs are publication-current and connector-verified. Never edit 0113 or 0114 after application, and never modify frozen 0110–0112 or recanonicalize their receipts. [Exact Content Package review](CONTENT_PACKAGE_REVIEW.md) is the authoritative schema/module/limit/error contract.
...
2. Apply reviewed migrations 0113 and 0114, in order, with the repository runner, then deploy the matching 1.24 readers, writers and UI together. Old writers omit mandatory proof/lineage and intentionally fail closed after 0113; 0114 additionally rejects a direct `learning_review.evidence_item_id = NULL` rewrite while its referenced evidence row still exists.
...
4. If deployment fails, keep writes closed and prefer a forward repair. A code-only rollback to 1.23 with 0113/0114 retained is unsafe; do not remove guards, null provenance or fabricate approvals. A database restore is a coordinated recovery that must reconcile every newer decision and external outcome.
...
Use `ContentPackageReviewRepository` and the transaction-scoped mutation/generation helpers; never assemble a trusted review from separate browser DTOs or bypass the package-root lock. Material evidence/conflict/accessibility/rights changes require both numeric-version and fingerprint preconditions, invalidate the current pointer and return one new coherent review. Proved approval, Learning Review and generation rows are immutable while their workspace survives. Do not clear a retained Learning Review's evidence pointer directly; 0114 allows it to become null only after the referenced evidence row is absent during the foreign-key source-refresh action. Fixture cleanup must delete the owned workspace/organization aggregate rather than directly deleting individual proof rows.
...
| `SOURCE_PREPARATION_BATCH_SIZE`     | Ingestion worker                   | Exact-approval draft-preparation commands claimed per sweep; integer from 1 through 100, default 10.                                                    |
| `SOURCE_PREPARATION_LEASE_SECONDS`  | Ingestion worker                   | Fenced command-processing lease; integer from 30 through 3,600 seconds, default 300. Expired attempts may be reclaimed and must replay idempotently.    |
...
Live acceptance fixtures use the exact `QA ... 0.13` names in a dedicated `QA Governed Drafts` workspace owned by the synthetic development user. The helper accepts only `seed`, `bridge`, or `clean`, rejects unknown commands before opening the database, and refuses any database whose URL path does not match `market_me_qa_*`. Run `$env:DATABASE_URL=...; npm run qa:governed-drafts -- seed`, select that workspace for browser checks, then run `npm run qa:governed-drafts -- clean`. Every command holds the session-level PostgreSQL advisory lock derived from `market-me:qa-governed-drafts:v1` on one reserved connection until all of its transactions finish, so concurrent helper processes serialize. Cleanup verifies the fixed organization/workspace markers, sole synthetic-user ownership and single-workspace boundary before deleting the complete disposable organization aggregate. It never deletes proof-bearing Draft, approval or Learning Review rows individually while their workspace survives.

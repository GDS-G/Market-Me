...
## Current candidate: 1.25 source-bound draft preparation
...
Migration 0115's `AFTER UPDATE OF current_approval_id` trigger is the transactional outbox boundary. A genuine new approval pointer plus an enabled binding inserts one `source_preparation_command` in the approval transaction; installation, binding creation, already-approved packages and same-receipt approval replay do not backfill. The immutable command pins relational workspace/source/package/version/approval/fingerprint/writer identity, binding revision and two JSON snapshots. `expected_approval_id` is globally unique and `preparation_idempotency_key = id`, so every explicit reapproval can create a new command while a later byte-identical receipt cannot silently satisfy an older unstarted command. Additive migration 0116 preserves disablement when unchanged saved references have become stale, revalidates all references before enablement, and hardens due/lease/attempt and exact completion guards without rewriting 0115.

The ingestion worker processes one command immediately after each claim rather than leasing an entire batch before work. `FOR UPDATE SKIP LOCKED` grants a bounded lease and increments `attempt_count`; that attempt number fences completion/failure. `SourcePreparationService` reconstructs authoritative workspace/package/version values from dedicated columns, uses the command UUID as the existing preparation idempotency key and passes the exact approval ID/fingerprint to `CampaignPreparationRepository.prepare()`. That UUID is a reserved internal namespace: the preparation repository permits it only for the processing command's exact writer, configuration, approval and fingerprint, and the web DTO does not expose it. Crashing after preparation commit but before command completion is safe. Before any ninth claim, an expired eighth attempt is reconciled directly to completed when an exact receipt exists, or to dead-letter when it does not. Deterministic policy/reference/lineage failures dead-letter; earlier transient failures use bounded exponential retry; a lost attempt may not settle its replacement.

The result is still **Prepared—not activated**: one Campaign, an internally published immutable `draft_only` planning version, governed drafts, preparation receipt and audits. That internal planning-version publication establishes immutable generation ancestry; it is not external publication and creates no Campaign instance, workflow command, finalization, approval, activation, schedule, `publication_action`, credential access, provider call or send. Existing exact-preview finalization and every downstream approval/execution boundary remain separate. [Source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md) defines the complete tables, DTOs, states, roles, API/UI and recovery contract.

The web boundary is a separate strict `GET`/`PUT /api/v1/smart-sources/:id/preparation-binding?workspaceId=...` route. GET returns only the minimized binding view; server-rendered source pages obtain minimized recent command summaries through a membership-scoped repository query. PUT requires exact Origin, one workspace query/body value, JSON no larger than 32,768 bytes, a strict unknown-field-free schema and current writer access. Browser views omit the command UUID plus writer/approval/fingerprint/idempotency/lease/snapshot authority fields. Candidate readiness expects 116 migrations ending at 0116. Frozen migration checksums, live replay/recovery proof, full local build/browser/native gates, Google development-document synchronization and the first feature run with both clean audits are observed; final evidence/reviewed commit, main cloud CI and publication remain pending.
...
The web form stores a tab-scoped exact attempt before submission, fetches coherent preview/token data and exposes publication/activation as separate explicit actions. No new service, background job, provider secret or mutable process-global request state is added. Verified 1.23 acceptance passes the 1,405-case full suite, browser/build/native checks and clean Linux CI; exact source/evidence is in [Releases](RELEASES.md), not a whole-product completion claim. Deploy compatible readers/admission/workers before finalization writers and exclude incompatible consumers from protected executions.

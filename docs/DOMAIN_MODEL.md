...
## 1.25 source-preparation binding and command identities (candidate)
...
| --- | --- |
| `smart_source_preparation_binding` | One mutable configuration per Smart Source. Source/workspace/creator identity is fixed; every mutation increments `revision` exactly once. `enabled` applies only to future approval transitions. Approval enqueue holds an observed enabled row through commit so a concurrent disable is ordered rather than silently canceling or inventing intent. |

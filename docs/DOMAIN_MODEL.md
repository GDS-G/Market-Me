...
The method returns the existing server-side `SourcePreparationCommandSummary | undefined`. The raw summary may contain command `id`, `expectedApprovalId`, optional `leaseExpiresAt`, `preparationId` and `campaignId`, so it must stay on the server. `sourcePreparationCommandView()` creates the closed browser shape `{status, bindingRevision, contentPackageId, contentPackageVersion, attemptCount, nextAttemptAt?, lastErrorCode?, safeError?, preparationId?, createdAt}`. Its omitted fields are a security contract, not a convenience. `preparationId` is navigation to an immutable receipt, never execution authority.

`SourcePreparationCommandStatus` has three props: `workspaceId`, minimized `command`, and optional presentation-only `showPackageLink`. It renders the existing five-state union without mutable state. No-command is modeled as `undefined`, not as a sixth pending status: command insertion and approval receipt creation commit atomically, so absence is an exact historical result and later binding changes do not backfill it.

## 1.25 source-preparation binding and command identities

`SourcePreparationBindingWrite` is the complete writer-authored configuration: `{workspaceId, smartSourceId, enabled, templateKey?, templateVersion?, name?, description?, brandProfileVersionId?, audienceProfileVersionIds?, destinationId?, informationDepth?, promotionalStrength?, timezone?}`. Normalization fixes the template to `general_announcement@1`, applies the established contextual/informational/UTC defaults, rejects unknown authority or execution fields, and retains authored Audience order. `StoredSourcePreparationBinding` adds the database UUID, authenticated `writerUserId`, monotonically increasing `revision`, `createdBy`/`updatedBy`, and database times. A save or enable/disable mutation makes the authenticated owner/admin/editor the current writer; `writerUserId` is never accepted from a browser body. New, changed or enabled reference sets require current published Brand/Audience versions and a published Destination. A disabled save may retain an identical reference-ID set that has since become stale, which preserves the operator's ability to stop future enqueue; re-enable validates the complete retained set.

The corresponding persistent relations have different lifetimes:

| Relation / field | Domain meaning |
| --- | --- |
| `smart_source_preparation_binding` | One mutable configuration per Smart Source. Source/workspace/creator identity is fixed; every mutation increments `revision` exactly once. `enabled` applies only to future approval transitions. Approval enqueue and concurrent enable/disable changes serialize on the binding lock, so the state observed in the approval transaction—not a previously loaded browser Boolean—decides whether that exact receipt gets a command. |
| `smart_source_preparation_binding_audience` | Normalized ordered Audience pins. `(binding_id, audience_profile_version_id)` prevents duplicates and `(binding_id, sort_order)` preserves one authored order in the closed range `0..19`. Empty means one general-audience variant. A deferred 0116 guard validates all retained Audiences at commit whenever the binding is enabled. |
| `source_preparation_command` | Durable approval-transaction outbox plus operational receipt. Its authority/configuration columns and JSON snapshots are immutable; only its fenced lifecycle fields may change until terminal. |
| `expected_approval_id` | Exact immutable Content Package approval that created the command. It is globally unique, so same-receipt replay cannot duplicate a command and a byte-identical later approval remains a distinct event. |
| `expected_review_fingerprint` | Exact reviewed bytes expected by preparation; it complements rather than replaces approval identity. |
| `configuration_snapshot` / `binding_snapshot` | Immutable normalized preparation input and human/audit binding context captured at enqueue. Dedicated relational workspace/package/version/approval/writer columns remain execution authority. |
| `preparation_idempotency_key` | Constrained equal to command `id`. This UUID is a hidden reserved internal namespace: a matching `campaign_preparation` may be inserted only for the processing command's exact writer, package/version, configuration, approval and fingerprint. Lease recovery therefore replays the same receipt rather than creating a second Campaign. |
| `attempt_count`, `claimed_at`, `lease_expires_at` | Durable claim generation and lease. The incremented attempt fences settlement; there is no in-memory ownership map. |
| `last_error_code`, `safe_error` | Bounded sanitized operations output. Raw driver, SQL, credential and provider detail must not be persisted or logged through this path. |
| `preparation_id`, `campaign_id`, `completed_at` | Terminal success lineage. The database guard verifies exact preparation key, package/version, configuration snapshot, configured writer, Campaign, approval ID and fingerprint before completion. |

`SourcePreparationCommandStatus` is the closed union `pending | processing | failed | completed | dead_letter`. Pending/due failed/expired processing rows below the cap may be claimed; claim increments the attempt and clears prior safe failure text. A current processing attempt may become completed, failed with a due time, or dead-lettered. `completed` and `dead_letter` are immutable. `completeSourcePreparationCommand()` returns `false` only for a lost `(status, attempt)` fence; invalid current-attempt lineage raises SQLSTATE `23514`. `failSourcePreparationCommand()` returns `completed` when it reconciles an exact committed receipt, `failed`, `dead_letter`, or `undefined` for a lost fence. With `SOURCE_PREPARATION_MAX_ATTEMPTS = 8`, attempts 1–7 schedule 5, 10, 20, 40, 80, 160 and 320-second retries; an eighth retryable failure is terminal. An expired eighth attempt is never reclaimed: an exact receipt is reconciled to completed, otherwise it dead-letters as `attempt_limit_exhausted`.

`ClaimedSourcePreparationCommand` exposes the exact relational identities, snapshots, attempt and lease only to the worker. `SourcePreparationCommandSummary` is membership-scoped server status; `SourcePreparationCommandView` further removes the command UUID, approval IDs, fingerprints, writer IDs, idempotency keys, lease times, Campaign ID and snapshots before rendering. `SourcePreparationBindingView` similarly omits writer/audit identities. Important immutable constants are `SOURCE_PREPARATION_COMMAND_LIMIT = 100`, `SOURCE_PREPARATION_DEFAULT_LEASE_SECONDS = 300`, `SOURCE_PREPARATION_MAX_ATTEMPTS = 8`, and the 32,768-byte web body limit. The worker environment narrows leases to `30..3600` seconds and batches to `1..100`; the repository's shorter internal lease allowance exists for controlled testing, not operator configuration.

`SourcePreparationBatchResult` is `{claimed, completed, retried, deadLettered, lost, settlementFailed}`. Its counters, the one-element `commands` claim result, classified `failure` dictionary and reconstructed configuration are invocation-local values used only for fixed structured logging/control flow. No process-global command, authorization, retry or idempotency dictionary exists. PostgreSQL rows, locks and constraints are durable truth. Preparation still yields only a planning Campaign, its internally published immutable `draft_only` version, governed drafts and a receipt; internal planning publication is not external publication or execution authority. Full contracts and release evidence are in [Source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md).

Migration `0115_source_bound_draft_preparation.sql` owns the base relations/outbox. Additive forward migration `0116_source_preparation_recovery_hardening.sql` owns the safe stale-reference disable/re-enable rules, deferred enabled-Audience validation, reserved command-key receipt guard, approval/disable ordering and exact capped recovery/completion checks. Release readiness is 116 migrations ending at 0116; frozen checksums, local release proof, Google-document readback, exact reviewed feature/main CI with clean audits and publication on `main` are verified.
...
`ExactPreviewSnapshotV1` is a strict detached raw-JSON structure with lineage, exact rendering/counts/capability snapshot/observation/creator, provider-specific connection identity, optional Destination and tracked link, and exactly empty assets. Every schema key is present; absent optional values are null. `ExactPreviewProvider`, `ExactPreviewConnectionV1`, `ExactPreviewRawJson` and `ExactPreviewRawJsonObject` describe its closed provider and recursive JSON contracts. `ExactPreviewFingerprintResult` is `{token,canonicalSnapshot,snapshot}`. Display-only labels are added by `ExactTextPreviewSelection`; credentials and health timestamps are excluded. SQL UTC microsecond strings must never pass through Date. The loader separately proves current eligibility and raw JSONB losslessness.

`EXACT_PREVIEW_FINGERPRINT_VERSION=1`, renderer contract `stored-channel-preview-text-v1`, domain `market-me:exact-preview:v1\n`, and prefix `mm-preview-v1:sha256:` define immutable byte semantics. Frozen limits are depth 32, nodes 50,000, string/key code units 262,144 and canonical UTF-8 bytes 1,048,576. Canonical object keys use direct UTF-16 sorted traversal; arrays retain order and strings retain exact spelling. Counters/ancestor Sets are call-local, not mutable globals. Preview token and canonical-input configuration hash have different domains and purposes.

Migration 0112's `campaign_finalization` stores unique workspace/key retry identity, unique preparation/Campaign/finalized-version lineage, selected draft/version/preview IDs, canonical snapshot/token, canonical intent/configuration hash, historical compiled definition, template version, actor and database timestamp. `StoredCampaignFinalization` exposes parsed raw snapshot/definition and six-digit UTC creation time but omits canonical intent. Receipts have no automatic expiry/deletion API. The prepared initial `{draftId,versionId}[]` remains history even when an approved reviewed copy revision is selected.

`FinalizedCampaignProof` is a transaction/request-local exact `{finalizationId,campaignId,campaignVersionId,campaignStepId,stepKey,previewId,previewFingerprint,channelConnectionId}` projection. `market_me.exact_preview_admission` is a PostgreSQL **transaction-local compatibility marker**, exactly `{finalizationId,campaignVersionId,campaignStepId,campaignInstanceId,campaignStepRunId,previewFingerprint}`. Common publication admission sets it only after locked authority/proof checks; commit/rollback clears it. It is not a secret, approval, process global or protection against a privileged SQL client. Durable Campaign provenance makes proof mandatory even if an old client removes a step token.

Browser `FinalizationAttempt={version:1,userId,idempotencyKey,input}` is tab-scoped response-loss recovery state under `market-me:campaign-finalization:v1:<user>:<workspace>:<preparation>`. It never supplies an authoritative snapshot. Selection sequence/in-flight/confirmation/error values are component-local only. Every exported API, error vocabulary, column, collection lifetime, lock/recovery rule and remaining acceptance gate is documented in [Campaign finalization](CAMPAIGN_FINALIZATION.md). Verified 1.23 release evidence is recorded in [Releases](RELEASES.md).

## 1.22 preparation and retry identities
...
Transaction-local profile Maps separate sorted lock order from user-authored variant order. New helper return values contain exact created IDs and never hydrate through mutable current pointers. `publishExactCampaignDraftInTransaction` only compares identity; 1.23 finalization instead rejects any existing editable draft and uses `insertPreparedExecutableDraftInTransaction` to create a distinct protected version. The shared `CommunicationPolicyInput` explicitly allows SQL-null settings; both most-specific selection and least-permissive-ceiling selection ignore null/undefined without ignoring a real lower ceiling.

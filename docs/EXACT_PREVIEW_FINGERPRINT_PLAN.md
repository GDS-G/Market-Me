...
Status: **implemented in the verified 1.23 local preview and Linux CI**. The pure primitive, raw loader, finalization compiler/repository, migration 0112 receipt/content/compatibility guards, and activation/activity/initial-and-retry publication proof checks are implemented. The current programmer contract is [Campaign finalization](CAMPAIGN_FINALIZATION.md); this document retains reviewed rationale and the acceptance checklist. Focused subsets are 134 fingerprint, 52 loader, 101 compiler, 57 finalization, 44 combined helper/fence and 30 common-proof cases within the 1,405-case full suite. Exact browser/build/native and cloud source evidence is in [Releases](RELEASES.md); none establishes whole-product or production acceptance. The 1.22 capability race repair remains a prerequisite.

...
## Current checkpoint: 1.19 implementation boundaries
...
### Worker rollout is not a transparent replay upgrade
...
Retry ownership is an atomic `failed -> dispatching` comparison-and-set: only its winner may send, and observers must not overwrite another active dispatch. Exact account, approved preview and current campaign authority are rechecked before claiming; missing/partial historical provider identity fails closed. This is not a claim that every mutable eligibility read and the publication claim share one transaction/lock hierarchy. Concurrent rights/eligibility revocation versus initial/retry dispatch remains a production-hardening requirement, and revocation cannot recall a provider request already in flight. Quiesce affected work when changing authority until that shared boundary is verified.
...
- Cancel signals interrupt active workflow cancellation scopes. Cleanup/status writes run in a non-cancellable scope so the database reflects terminal cancellation even when a timer or activity was in flight.

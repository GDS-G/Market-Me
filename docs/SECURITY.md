...
The PUT boundary requires exact configured Origin, `application/json`, a streaming maximum 32,768-byte body, one nonduplicated `workspaceId` query parameter matching the body, canonical UUIDs and a strict unknown-field-free schema. Allowed configuration is limited to `general_announcement@1`, bounded name/description/timezone, optional current published Brand, at most 20 unique ordered current published Audiences, optional published Destination, copy controls and `enabled`. Actor/writer IDs, approval IDs/fingerprints, command status, snapshots, leases, error/result lineage, arbitrary Campaign graph, account/credential fields and approval flags are not writable input. Responses are `no-store`; browser models omit the command UUID, configured writer, exact approval/fingerprint, idempotency key, lease, Campaign ID and snapshots.

The approval transaction—not browser or worker memory—is the outbox trust boundary. A new non-null `content_package.current_approval_id` transition plus an enabled source binding inserts one immutable command with exact workspace/source/package/version/approval/fingerprint/writer/binding-revision columns and normalized JSON snapshots. The database verifies those columns against the binding/package/receipt and constrains `preparation_idempotency_key = id`; globally unique `expected_approval_id` prevents duplicate enqueue from replay. Migration 0116 holds the observed binding `FOR SHARE` through approval commit. If disable commits first, approval sees disabled and enqueues nothing; if approval observes enabled first, disable waits and cannot erase that intent. No migration/binding-time backfill exists. A later explicit byte-identical approval is a new security event with a different receipt and command; the old command may not silently adopt it.

Binding snapshots are evidence, not continuing authority. At execution `CampaignPreparationRepository.prepare()` rechecks the configured writer's current writer role, exact current approval ID/version/fingerprint, current published profiles/Destination and communication policy under existing locks. Revocation, stale reference or superseding approval therefore fails closed and creates no partial Campaign. New, changed or enabled bindings validate current references; a disabled save may retain the identical saved reference-ID set after it becomes stale so the safe-stop control remains usable, but re-enable proactively and defer-validates all retained Brand/Audience/Destination references. Dedicated relational command columns override JSON workspace/package/version values. Editing/disabling the binding or changing source synchronization state after enqueue cannot rewrite or cancel the immutable command; use normal terminal evidence and a new explicit approval for later intent.

The command UUID is a hidden reserved preparation-idempotency namespace, not a browser capability. Before replay or insertion the preparation repository locks any matching source command `FOR SHARE` and requires `processing` plus its exact writer, package/version, configuration snapshot, approval ID and fingerprint. Migration 0116 enforces the same predicate on `campaign_preparation` insert. A manual caller with a guessed or leaked UUID therefore cannot preempt a pending command, reuse a terminal command or substitute different settings.

`FOR UPDATE SKIP LOCKED`, leases and incrementing attempts coordinate workers without a process-global owner map. The `(processing, attempt_count)` predicate fences completion/failure after lease recovery. Claims require an actually due pending/failed row or expired processing lease and stop at eight. UUID reuse makes a crash after preparation commit safe: the original preparation receipt replays without another Campaign. A retryable response/settlement loss first reconciles an exact receipt; an expired eighth attempt does the same, otherwise it dead-letters as `attempt_limit_exhausted` without a ninth claim. Invalid current-attempt completion lineage, including nonidentical configuration, raises `23514` and becomes terminal unless exact committed receipt recovery applies. Connection/serialization/deadlock/lock/capacity/timeout failures after attempts 1–7 use 5, 10, 20, 40, 80, 160 and 320-second delays. Stored `last_error_code`/`safe_error` and worker logs are closed, bounded categories; raw SQL, identifiers, connection strings, provider messages and credentials must never flow into them. A settlement-store failure leaves the lease for recovery rather than guessing an outcome.
...
Migrations 0115–0116 and their guards assume a trusted, least-privilege application database role; a privileged SQL client or database owner remains outside the application authority model. Coordinated rollout is required because old approval writers can fire the new trigger but old ingestion workers cannot consume its commands. Stop writers, verify/backup, apply 0115 then 0116, deploy matching code and reopen only after end-to-end checks. Candidate readiness expects 116 migrations ending at 0116. Frozen migration checksums, live integration/recovery evidence and full local/browser/native gates are observed; reviewed commit, public-workflow audits, feature/main cloud CI and publication remain pending. See [Source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md), [Developer Guide](DEVELOPMENT.md) and [Recovery](RECOVERY.md).

## Current implementation: 1.24 exact Content Package review boundaries

Every package review/read, material action and approval resolves the current authenticated user and exact workspace. All six workspace roles may read current and historical review; owner/admin/editor may change asset accessibility or rights and create new work; owner/admin/approver may resolve evidence/conflicts or approve. The repository repeats authorization under ancestor-first locks. A fingerprint, receipt UUID, idempotency key, browser role or legacy `approved` flag is never a bearer permission.

The server captures one bounded raw package/evidence/conflict/asset projection and rejects lossy JSON numbers, malformed microsecond times, invalid graph decisions and partial/oversized snapshots. The domain-separated SHA-256 fingerprint proves equality to retained canonical bytes, not truth, malware safety, copyright, current provider access or a signature. Approval separately rechecks clean scan evidence, reviewed accessibility and recorded current rights/expiry using fresh database time; captured provider names and nonempty permitted-connection references remain evidence, not a live provider/account authorization check. Material evidence, conflict, accessibility and rights actions invalidate the current receipt; new generation requires the exact current receipt and its full effective-evidence set. Existing Draft, Campaign, account, preview and provider authorization remains independent and must revalidate live connection/capability authority downstream.

Approval, Learning Review proof, current pointer/status and audit commit atomically. The private `content_package_approval.canonical_review_snapshot` retains the full claims, extracted text, metadata/object keys and rights owner/proof/notes/scopes; `learning_review_proof.decision_snapshot` retains the action-specific decision (for approval, the receipt ID and effective-evidence IDs). General `audit_event.data` is deliberately minimized to stable IDs, before/after fingerprints and statuses, permission booleans, scope/count/presence facts and lengths; it must not duplicate raw alt text, notes, owner/license data, proof references, scope arrays or complete claims. Apply database, backup, export, support and retention access controls to both stores rather than treating the decision proof alone as the package snapshot.

Deferred guards reject update, retarget, delete/reinsert and ordinary package-level deletion of proved decisions, receipts and generations while the workspace survives. Only an authorized whole-workspace or owning-organization cascade may erase that history. This is an integrity/retention boundary, not an independent authorization system or tamper-proof ledger: current live repository checks remain authoritative, and triggers plus the transaction-local `market_me.content_package_approval_admission` marker assume a trusted, least-privilege application database role. A caller with arbitrary SQL as that role can set a custom GUC; a database owner can disable the controls. Production must separate migration/administrative identities, restrict runtime SQL and monitor privileged access.

Migration 0113 requires stop-the-world writer coordination. Old review/generation/preparation/finalization writers fail closed after it, and a code-only rollback to 1.23 is unsafe for ordinary writes. Preserve immutable source and external-outcome history, prefer a forward repair and follow [Exact Content Package review](CONTENT_PACKAGE_REVIEW.md), [Developer Guide](DEVELOPMENT.md) and [Recovery](RECOVERY.md). Final local integrated 1.24 acceptance, Google documentation readback, exact feature/main Linux cloud CI and reviewed-source publication on `main` passed; this section is not production certification.

## Verified 1.23 exact-preview finalization boundaries
...
- Release 1.24 additionally requires the exact package review fingerprint and immutable approval receipt described in [Exact Content Package review](CONTENT_PACKAGE_REVIEW.md); numeric version, current pointer and UI state are insufficient independently.

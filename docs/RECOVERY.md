...
## Release 1.25 source-preparation recovery note (candidate)
...
Keep web approval writes and ingestion workers stopped while restoring. Verify frozen migrations 0113–0114 plus base migration 0115 and additive hardening migration 0116/checksums against the deployed code, then validate every restored command by state before reopening. Candidate readiness must report exactly 116 migrations ending at `0116_source_preparation_recovery_hardening.sql`:

- `pending` and due `failed` commands are eligible only through the normal claim path after current writer, exact current approval and current profile/Destination eligibility are revalidated by preparation.
- `processing` is not proof that preparation did or did not commit. Preserve `attempt_count`, `claimed_at` and `lease_expires_at`. After normal expiry an attempt below eight may be reclaimed with the same command UUID. An expired eighth attempt is never claimed a ninth time: the worker reconciles an exact existing receipt to completed or records `attempt_limit_exhausted` as a dead letter when no receipt exists.
- `completed` must retain matching `campaign_preparation` and Campaign lineage for the same idempotency key, package/version, exact configuration snapshot, configured writer, exact approval ID and fingerprint. A missing/mismatched result is corruption to investigate, not permission to rewrite the command.
- `dead_letter` is immutable terminal evidence. Correct live configuration and record a new explicit exact approval only when a person intends new work; do not requeue or convert the historical row.

Do not bulk-clear leases, attempts, error codes, completion times or result IDs. A stale attempt is fenced by `(processing, attempt_count)` and must not settle a recovered claim. If command settlement was unavailable at the recovery point, leave the existing lease to expire. Retryable failure settlement first reconciles an exact committed receipt, including after completion-response loss; do not manually synthesize that result. Raw database/provider/credential errors do not belong in `safe_error` or recovery logs. Restore validation should also prove that `preparation_idempotency_key = id`, `expected_approval_id` remains unique, terminal rows are immutable and the command's dedicated relational identity agrees with its retained snapshots. The command UUID is a hidden reserved namespace: a matching preparation insert is valid only for a processing command with exact writer, package/version, configuration, approval and fingerprint.

An enabled binding does not backfill restored approved packages. It affects only a later new `current_approval_id` transition; `smart_source.enabled` remains a separate synchronization control. A restored disabled binding may legitimately retain now-stale Brand/Audience/Destination IDs. Keep it disabled until the complete set is current or replaced; do not bypass the 0116 re-enable validation. Before reopening approval writes, confirm operators understand that each explicit reapproval may create a distinct command even when review bytes match, binding edit/disable cannot cancel a queued snapshot, and concurrent approval/disable must retain the migration's commit order. If the 1.25 code deployment fails after schema application, stop binding creation and approval traffic, retain all new rows and prefer a forward repair; an old web writer can still fire the trigger while an old ingestion worker cannot consume the outbox.

Source-bound recovery has no external-send ambiguity of its own. A successful command creates only a Campaign, internally published immutable `draft_only` planning version, governed drafts and preparation receipt. That internal planning-version publication is not external publication and creates no instance, workflow command, activation, schedule, `publication_action`, provider call or send. Downstream finalization/publication evidence must still be reconciled under its separate runbooks. Both candidate migration checksums, live replay/recovery validation and the first feature run with both clean audits are observed; final evidence/reviewed commit, main cloud CI and Release 1.25 publication remain pending. See [Source-bound draft preparation](SOURCE_BOUND_DRAFT_PREPARATION.md), [Campaign preparation](CAMPAIGN_PREPARATION.md), [Security](SECURITY.md) and [Developer Guide](DEVELOPMENT.md).

## Release 1.24 exact package-review recovery note (development)

Restore `content_package`, its evidence/conflicts/assets/rights scopes, `content_package_approval`, `learning_review`, `learning_review_proof`, proof-bearing `draft_generation`, Campaign preparation/finalization receipts and their referenced users/workspaces as one relational evidence set. Preserve exact canonical snapshot/input text, hashes, UUID arrays, microsecond timestamps and original receipt/generation links. Never regenerate a receipt from current rows, recanonicalize retained JSON, retarget an old proof, attach proof to a legacy null generation or delete/reinsert history to satisfy a constraint.

Keep all 1.23/older mutation, review, generation, preparation and finalization writers stopped. Restore into a new isolated target, apply/verify migration 0113 and its frozen checksum, and use only matching 1.24 readers/writers for validation. A current pointer or restored `approved` status is not permission: load the exact snapshot through the 1.24 repository, validate receipt bytes/effective evidence, recheck current scan/accessibility/rights/account/time eligibility with fresh database time and create controlled new work only after those checks. Historical completed recovery may return its original receipt; it must not be mistaken for renewed authority.

The deferred deletion guards intentionally allow only whole-workspace or owning-organization erasure. Individual package, receipt, Learning Review/proof and proved-generation deletion is rejected while the workspace survives. Do not disable those guards for partial restore, test cleanup or retention work. If approved policy requires workspace erasure, coordinate PostgreSQL, object storage, backups, Temporal and external providers as one operation, record the authorization and verify no cross-workspace rows were affected.

The transaction-local admission marker and trigger checks assume a trusted least-privilege application database role; they do not make arbitrary SQL or superuser access safe. Validate recovery with the production runtime role as well as the controlled migration role, review privileged-access evidence and keep broad audit logs minimized. The exact private approval/decision snapshots—not general audit dictionaries—hold source claims, extracted text and rights details. Protect them in backups/exports and preserve them unless the authorized whole-workspace lifecycle applies. See [Exact Content Package review](CONTENT_PACKAGE_REVIEW.md), [Security](SECURITY.md) and [Developer Guide](DEVELOPMENT.md). The 1.24 release gate is verified; deployment-specific production restore acceptance remains separate.

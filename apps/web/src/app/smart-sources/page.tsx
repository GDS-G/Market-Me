...
import { getRepository, getSourcePreparationRepository } from "@/server/database";
...
export default async function SmartSourcesPage() {
  ...
  const [sources, preparationBindings] = await Promise.all([
    repository.listSmartSources(workspace.workspaceId),
    getSourcePreparationRepository().listSourcePreparationBindings(workspace.workspaceId, user.id),
  ]);
  const preparationBySource = new Map(preparationBindings.map((binding) => [binding.smartSourceId, binding]));
        ...
        <section className="resource-panel">
          ...
          {sources.length === 0 ? (
            ...
            <div className="resource-table">
              {sources.map((source) => {
                const binding = preparationBySource.get(source.id);
                return <article className="resource-row" key={source.id}>
                  ...
                  <div><span className="resource-label">Readiness</span><strong>{source.readinessMode.replaceAll("_", " ")}</strong>
                    <span className="resource-label" style={{ marginTop: 7 }}>On exact approval</span><strong>{binding ? binding.enabled ? "Draft prep enabled" : "Draft prep paused" : "Not configured"}</strong></div>
                  <span className={`status-pill ${source.enabled ? "status-green" : "status-neutral"}`}>{source.enabled ? "Sync enabled" : "Sync paused"}</span>

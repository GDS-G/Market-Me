...
import { getCampaignRepository, getContentPackageReviewRepository, getProfileRepository, getPublishingRepository } from "@/server/database";
...
export default async function ContentPackagePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [channelConnections, campaigns, brands, approvals] = await Promise.all([

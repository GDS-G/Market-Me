...
import { ArrowLeft, CheckCircle2, Database, Sparkles, TriangleAlert } from "lucide-react";
    ...
    <div className="resource-page form-page draft-detail">
      ...
      <section className="resource-panel draft-trace">
        ...
        <div className="claim-list">{version.claims.map((claim) => {
          const trace = describeDraftClaimTrace(claim, draft.generation.evidenceSnapshot);
          return <article key={claim.id}>
            <span>{trace.status === "unavailable" ? <TriangleAlert size={16} /> : trace.status === "presentation" ? <Sparkles size={16} /> : <CheckCircle2 size={16} />}</span>
            <div><strong>{claim.text}</strong><p>{trace.text}</p></div>
          </article>;
        })}</div>
      </section>

...
import { conversationHandoffWriteSchema } from "@/server/conversation-schema";
...
export async function POST(
  ...
  try {
    ...
    const parsed = conversationHandoffWriteSchema.safeParse(
      await request.json(),
    );
      ...
      return Response.json(
        {
          error: {
            ...
            message: "Check the handoff brief fields.",
    ...
    const data = await getConversationRepository().createHandoff(
      parsed.data,
      user.id,
    );
    return data
